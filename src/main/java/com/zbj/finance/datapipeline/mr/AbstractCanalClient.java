...
//        Assert.notNull(connector, "connector is null");
            ...
            if (entry.getEntryType() == EntryType.ROWDATA) {
                ...
                for (RowData rowData : rowChage.getRowDatasList()) {
                    if (eventType == EventType.DELETE) {
                        processColumns(rowData.getBeforeColumnsList(), entry, rowChage);
                    } else if (eventType == EventType.INSERT) {
                        processColumns(rowData.getAfterColumnsList(), entry, rowChage);
                    } else {
                        processColumns(rowData.getAfterColumnsList(), entry, rowChage);
                    }
                }
    ...
    protected void processColumns(List<Column> columns, CanalEntry.Entry entry, CanalEntry.RowChange rowChange) {
        ...
        key.append(entry.getHeader().getSchemaName() + "|");
        key.append(entry.getHeader().getTableName());
        ...
        for (Column column : columns) {
            ...
            if (column.getIsKey()) {
                builder.append(",isKey=" + column.getIsKey());
                key.append("|" + column.getValue());
            } else {
                builder.append(",isKey=false");
            }
        }

        pushToExternalSystem(key.toString(), builder.toString());
    }
